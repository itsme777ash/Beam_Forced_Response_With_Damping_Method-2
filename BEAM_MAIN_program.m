E = [210e9,210e9]; q = 0; P = 100; M = 0; density = [7850,7850];b = [0.05,0.06]; h = [0.01,0.02]; I =b.*(h.^3)/12;
damp_ratio = 0.1;
...
Y_val = zeros(40,1);
Amp_Y_real = zeros(length(5:1:100),1);
for f = 5:1:100
    omeg = 2*pi*f;
    for i = 1:1:40
    ...
    temp_amp_per_mode = 0;
    for j = 1:1:40
        temp_amp_per_mode = temp_amp_per_mode + ((V(39,j)*Y_val(j))^2);
    end
   Amp_Y_real(f-4,1) = sqrt(temp_amp_per_mode)/W(DOF-1);
end
...
plot(linspace(5,100,96),Amp_Y_real);
